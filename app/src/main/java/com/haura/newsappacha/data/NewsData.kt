package com.haura.newsappacha.data
...
object NewsData {
    ...
    private val newsDesc = arrayOf(
        ...
        "Apple terus mengembangkan mobil listriknya. Dikutip dari Reuters, kemungkinan misi besar tersebut akan dirilis tahun 2025. Konon katanya mobil listrik milik Apple tanpa setir dan pedal.",
        ...
        "Pemberlakuan Pembatasan Kegiatan Masyarakat atau PPKM Level 3 di seluruh Indonesia diterapkan pada 24 Desember 2021 hingga 2 Januari 2022. Masyarakat diimbau tidak mudik."
    )

    val listData : ArrayList<NewsModel>
        get() {
            val list = arrayListOf<NewsModel>()
            for (position in newsImage.indices){
                val news = NewsModel()
                news.image = newsImage[position]
                news.title = newsTitle[position]
                news.desc = newsDesc[position]
                list.add(news)
            }
            return list
        }
